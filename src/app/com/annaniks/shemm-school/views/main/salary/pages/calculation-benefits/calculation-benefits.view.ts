...
import { ServerResponse, DataCount, Benefits } from 'src/app/com/annaniks/shemm-school/models/global.models';
import { map } from 'rxjs/operators';
import { CalculationBenefitsModal } from '../../modals';
import { DatePipe } from '@angular/common';

@Component({
    selector: 'calculation-benefits-view',
    templateUrl: 'calculation-benefits.view.html',
    styleUrls: ['calculation-benefits.view.scss']
})
export class CalculationBenefitsView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Նպաստների հաշվարկ';
    public calculationBenefits:Benefits[] = []
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getcalculationBenefitsCount(),
            this._getcalculationBenefits(limit, offset)
        )
    ...
    private _getcalculationBenefitsCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.calculateBenefitsMainUrl).pipe(
    ...
    private _getcalculationBenefits(limit: number, offset: number): Observable<ServerResponse<Benefits[]>> {
        return this._mainService.getByUrl(this._urls.calculateBenefitsMainUrl, limit, offset).pipe(
            map((data: ServerResponse<Benefits[]>) => {
                this.calculationBenefits = data.data;
                return data
            })
        )
    }
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(CalculationBenefitsModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.calculateBenefitsGetOneUrl, id: id }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.calculateBenefitsGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.calculationBenefits, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
            this._loadingService.hideLoading()
        })
    }
    public getDate(item:Benefits):string {
