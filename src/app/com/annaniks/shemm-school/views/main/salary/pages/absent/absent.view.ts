...
import { AbsencesPayload, ServerResponse, DataCount } from 'src/app/com/annaniks/shemm-school/models/global.models';
...
import { AddAbsencesComponent } from '../../modals';

@Component({
    selector: 'app-absent',
    templateUrl: './absent.view.html',
    styleUrls: ['./absent.view.scss']
})
export class AbsentView implements OnInit {

    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Բացականեր'
    private _paginatorLastPageNumber: number = 0;
    public absences: AbsencesPayload[] = [];
    ...
    private _combineObservable(limit: number, offset: number) {
        ...
        const combine = forkJoin(
            this._getAbsencesCount(),
            this._getAbsences(limit, offset)
        )
    ...
    private _getAbsencesCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.absencesMainUrl).pipe(
    ...
    private _getAbsences(limit: number, offset: number): Observable<ServerResponse<AbsencesPayload[]>> {
        return this._mainService.getByUrl(this._urls.absencesMainUrl, limit, offset).pipe(
    ...
    public addAbsent(isNew?: boolean, id?: number, item?): void {
    ...
    public openModal(isNew?: boolean, id?: number, item?): void {
        ...
        let dialog = this._matDialog.open(AddAbsencesComponent, {
            width: '700px',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.absenceGetOneUrl, id: id, item: item }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.absenceGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.absences, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
            this._loadingService.hideLoading()
        }, () => { this._loadingService.hideLoading() }
