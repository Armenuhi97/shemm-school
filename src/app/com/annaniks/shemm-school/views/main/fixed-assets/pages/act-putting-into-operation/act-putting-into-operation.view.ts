...
import { ActPuttingIntoOperationModal } from '../../modals';

@Component({
    selector: 'act-putting-into-operation-view',
    templateUrl: 'act-putting-into-operation.view.html',
    styleUrls: ['act-putting-into-operation.view.scss']
})
export class ActPuttingIntoOperationView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Շահագործման հանձնման ակտ';
    public actPuttingIntooperations=[]
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getActPuttingIntoOperationCount(),
            this._getActPuttingIntoOperation(limit, offset)
        )
    ...
    private _getActPuttingIntoOperationCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.actPuttingIntoOperationMainUrl).pipe(
    ...
    private _getActPuttingIntoOperation(limit: number, offset: number): Observable<ServerResponse<any[]>> {
        return this._mainService.getByUrl(this._urls.actPuttingIntoOperationMainUrl, limit, offset).pipe(
    ...
    public addActPuttingIntoOperation(isNew: boolean, id?: number): void {
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(ActPuttingIntoOperationModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
