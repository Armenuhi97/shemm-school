...
import { Partner, AnalyticalGroup, AccountPlans, ModalDataModel, ServerResponse, CashRegisterPayload, Currency, DataCount, JsonObjectType, GenerateType } from 'src/app/com/annaniks/shemm-school/models/global.models';
...
@Component({
    selector: 'cash-entry-order-modal',
    templateUrl: 'cash-entry-order.modal.html',
    styleUrls: ['cash-entry-order.modal.scss']
})
export class CashEntryOrderModal {
    ...
    constructor(private _dialogRef: MatDialogRef<CashEntryOrderModal>,
    ...
    private _getOperationArray(body): void {
        this._mainService.getOperationArray(this._urls.cashEntryOrderFunctionUrl, body, this.cashEntryOrderGroup, this._fb)
    }
    private _setForOperationObject() {
        let object = {
            cashRegisterId: this._appService.checkProperty(this.cashEntryOrderGroup.get('boxOffice').value, 'id'),
            date: this._datePipe.transform((this.cashEntryOrderGroup.get('date').value), 'yyyy-MM-dd'),
            documentNumber: this.cashEntryOrderGroup.get('folderNumber').value,
            hdmN: this.cashEntryOrderGroup.get('cashVoucherNumber').value,
            correspondentAccountId: this._appService.checkProperty(this.cashEntryOrderGroup.get('correspondentAccount').value, 'id'),
            entryAccountId: this._appService.checkProperty(this.cashEntryOrderGroup.get('dramAccount').value, 'id'),
            analiticGroup_1Id: this._appService.checkProperty(this.cashEntryOrderGroup.get('analyticalGroup1').value, 'id'),
            analiticGroup_2Id: this._appService.checkProperty(this.cashEntryOrderGroup.get('analyticalGroup2').value, 'id'),
            currencyId: this._appService.checkProperty(this.cashEntryOrderGroup.get('currency').value, 'id'),
            amountCurrency1: this.cashEntryOrderGroup.get('amountcurrency').value,
            amountCurrency2: this.cashEntryOrderGroup.get('ADM').value,
            partnersId: this._appService.checkProperty(this.cashEntryOrderGroup.get('partner').value, 'id'),
            received: this.cashEntryOrderGroup.get('getsFrom').value,
            basis: this.cashEntryOrderGroup.get('base').value,
            attached: this.cashEntryOrderGroup.get('attach').value,
            optiona: 'string',
    ...
    private _setDataFromTabs(): void {
        this._subscription1 = this._componentDataService.getDataState().subscribe((data) => {
            if (data) {
                this.cashEntryOrderGroup.get(data.type).setValue(data.data);
            }
    ...
    private _generateDocumentNumber(): void {
        if (!this._data.id) {
          this._mainService.generateField('cash_register_entry', 'document_number').subscribe((data: ServerResponse<GenerateType>) => {
            this.cashEntryOrderGroup.get('folderNumber').setValue(data.data.message.maxColumValue);
    ...
    private _validate() {
        this.cashEntryOrderGroup = this._fb.group({
            ...
            partner: [null],
            getsFrom: [null, Validators.required],
            base: [null],
            attach: [null],
            operation: [null]
        })
    ...
    public save(): void {
        ...
        let operationArray = []
        if (this.cashEntryOrderGroup.get('operation') && this.cashEntryOrderGroup.get('operation').value) {
            this.cashEntryOrderGroup.get('operation').value.forEach((element) => {
                let data = element.value;
                let object = this._appService.getOperationObject(data)
                operationArray.push(object)
            })
        }
        this._appService.markFormGroupTouched(this.cashEntryOrderGroup);
        let sendObject = {};
        sendObject['main'] = this._setForOperationObject()
        sendObject['cashRegisterEntryFunctions'] = operationArray;
        if (this.cashEntryOrderGroup.valid) {
            this._loadingService.showLoading()
            if (!this._data.id) {
    ...
    public setValue(event, controlName: string): void {
        this.cashEntryOrderGroup.get(controlName).setValue(event);
        this.onFocus(this.cashEntryOrderGroup, controlName)

    }
    public setInputValue(controlName: string, property: string) {
        return this._appService.setInputValue(this.cashEntryOrderGroup, controlName, property)
    }
