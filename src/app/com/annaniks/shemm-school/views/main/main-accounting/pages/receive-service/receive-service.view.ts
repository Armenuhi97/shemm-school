...
import { ReceiveServiceModal } from '../../modals';
...
import { ServerResponse, DataCount, ReceiveService } from 'src/app/com/annaniks/shemm-school/models/global.models';
import { map } from 'rxjs/operators';

@Component({
    selector: 'receive-service-view',
    templateUrl: 'receive-service.view.html',
    styleUrls: ['receive-service.view.scss']
})
export class ReceiveServiceView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Ստացված ծառայություններ';
    public receiveServices:ReceiveService[] = []
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getReceiveServiceCount(),
            this._getReceiveService(limit, offset)
        )
    ...
    private _getReceiveServiceCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.receiveServiceMainUrl).pipe(
    ...
    private _getReceiveService(limit: number, offset: number): Observable<ServerResponse<ReceiveService[]>> {
        return this._mainService.getByUrl(this._urls.receiveServiceMainUrl, limit, offset).pipe(
            map((data: ServerResponse<ReceiveService[]>) => {                
                this.receiveServices = data.data;
                return data
            })
        )
    }
    public addReceiveService(isNew: boolean, id?: number): void {
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(ReceiveServiceModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.receiveServiceGetOneUrl, id: id }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.receiveServiceGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.receiveServices, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
