...
import { FinalySettlementModal } from '../../modals';

@Component({
    selector: 'final-calculations-view',
    templateUrl: 'final-calculations.view.html',
    styleUrls: ['final-calculations.view.scss']
})
export class FinalCalculationsView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Վերջնահաշվարկ';
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getFinalCalculationsCount(),
            this._getFinalCalculations(limit, offset)
        )
    ...
    private _getFinalCalculationsCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.finalCalculationMainUrl).pipe(
    ...
    private _getFinalCalculations(limit: number, offset: number): Observable<ServerResponse<any[]>> {
        return this._mainService.getByUrl(this._urls.finalCalculationMainUrl, limit, offset).pipe(
            map((data: ServerResponse<any[]>) => {
                this.finalCalculations = data.data;
                return data
            })
        )
    }
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(FinalySettlementModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.finalCalculationGetOneUrl, id: id }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.finalCalculationGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.finalCalculations, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
