...
import { ServerResponse, DataCount, Vacations } from 'src/app/com/annaniks/shemm-school/models/global.models';
import { CalculationVacationTimeModal } from '../../modals';
import { DatePipe } from '@angular/common';

@Component({
    selector: 'calculation-vacation-time-view',
    templateUrl: 'calculation-vacation-time.view.html',
    styleUrls: ['calculation-vacation-time.view.scss']
})
export class CalculationVacationTimeView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Արձակուրդային հաշվարկ';
    public calculationVacationTimes:Vacations[] = []
    ...
    public getDate(item:Vacations):string {
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getCalculationVacationTimeCount(),
            this._getCalculationVacationTime(limit, offset)
        )
    ...
    private _getCalculationVacationTimeCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.calculationVacationTimeMainUrl).pipe(
    ...
    private _getCalculationVacationTime(limit: number, offset: number): Observable<ServerResponse<Vacations[]>> {
        return this._mainService.getByUrl(this._urls.calculationVacationTimeMainUrl, limit, offset).pipe(
            map((data: ServerResponse<Vacations[]>) => {
                this.calculationVacationTimes = data.data;
                return data
            })
        )
    }
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(CalculationVacationTimeModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.calculationVacationTimeGetOneUrl, id: id }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.calculationVacationTimeGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.calculationVacationTimes, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
