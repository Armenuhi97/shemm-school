...
@Component({
    selector: 'replenishment-view',
    templateUrl: 'replenishment.view.html',
    styleUrls: ['replenishment.view.scss']
})
export class ReplenishmentView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Համալրում';
    public replenishment: any[] = []
    ...
    private _getEnterVaultCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.replenishmentMainUrl).pipe(
    ...
    private _getEnterVault(limit: number, offset: number): Observable<ServerResponse<any[]>> {
        return this._mainService.getByUrl(this._urls.replenishmentMainUrl, limit, offset).pipe(
            map((data: ServerResponse<any[]>) => {
                this.replenishment = data.data;
                return data
            })
        )
    }
    ...
    private _openModal(isNew?: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(ReplenishmentModal, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
            data: { title: isNewTitle, url: this._urls.replenishmentGetOneUrl, id: id }
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.replenishmentGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.replenishment, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
