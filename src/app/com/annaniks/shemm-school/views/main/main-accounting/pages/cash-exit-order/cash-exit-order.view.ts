...
import { CashRegisterLeaveOrder } from '../../modals';

@Component({
    selector: 'cash-exit-order-view',
    templateUrl: 'cash-exit-order.view.html',
    styleUrls: ['cash-exit-order.view.scss']
})
export class CashExitOrderView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Դրամարկղի ելքի օրդեր';
    public cashExitOrders = []
    ...
    private _combineObservable(limit: number, offset: number): void {
        ...
        const combine = forkJoin(
            this._getCashExitOrderCount(),
            this._getCashExitOrder(limit, offset)
        )
    ...
    private _getCashExitOrderCount(): Observable<ServerResponse<DataCount>> {
        return this._mainService.getCount(this._urls.cashExitOrderMainUrl).pipe(
    ...
    private _getCashExitOrder(limit: number, offset: number): Observable<ServerResponse<any[]>> {
        return this._mainService.getByUrl(this._urls.cashExitOrderMainUrl, limit, offset).pipe(
    ...
    public addCashExitOrder(isNew: boolean, id?: number): void {
    ...
    private _openModal(isNew: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(CashRegisterLeaveOrder, {
            width: '80vw',
            minHeight: '55vh',
            maxHeight: '85vh',
            autoFocus: false,
