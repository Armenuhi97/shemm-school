...
import { ServerResponse, DataCount, Types } from 'src/app/com/annaniks/shemm-school/models/global.models';
import { AddTypesModal } from '../../modals';

@Component({
    selector: 'types-view',
    templateUrl: 'types.view.html',
    styleUrls: ['types.view.scss']
})
export class TypesView {
    private _pageLength: number = 10;
    private _count: number = 0;
    private _page: number = 1;
    private _modalTitle: string = 'Տեսակ';
    public types: Types[] = []
    ...
    private _getTypesCount(): Observable<ServerResponse<DataCount>> {
    ...
    private _getTypes(limit: number, offset: number): Observable<ServerResponse<Types[]>> {
        return this._mainService.getByUrl(this._urls.typesMainUrl, limit, offset).pipe(
    ...
    public addType(isNew?: boolean, id?: number): void {
    ...
    public openModal(isNew?: boolean, id?: number): void {
        ...
        let dialog = this._matDialog.open(AddTypesModal, {
            width: '500px',
            data: { title: isNewTitle, url: this._urls.typesGetOneUrl, id: id },
            autoFocus:false
        })
    ...
    public delete(id: number): void {
        this._loadingService.showLoading()
        this._mainService.deleteByUrl(this._urls.typesGetOneUrl, id).subscribe((data) => {
            let page = this._appService.setAfterDeletedPage(this.types, this._page)
            this._router.navigate([], { relativeTo: this._activatedRoute, queryParams: { page: page }, })
            this._combineObservable(this._pageLength, (page - 1) * this._pageLength)
